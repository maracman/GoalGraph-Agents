#!/usr/bin/env python3
"""Does a proven path transfer to an adjacent goal? The framing's own test.
...
ARMS = ['fresh', 'same', 'adjacent', 'mismatch']
...
def main():
    ...
    ap.add_argument('--donor-same', required=False, default='')
    ...
    ap.add_argument('--out', default='studies/results/v3_transfer_two.json')
    ...
    if not a.donor_same or not a.donor_adjacent:
        sys.exit('need --donor-same and --donor-adjacent graph ids')
    DONORS['same'] = a.donor_same
