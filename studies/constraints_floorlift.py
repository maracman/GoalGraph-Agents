#!/usr/bin/env python3
"""Confirmatory: a donor graph lifts a floored rider - measured at the right grain.

Two rounds of the existence proof converged on a mechanism nobody registered:
donor graphs transmit the WINNING TEMPLATE (a sentence shape satisfying all
five rules), whether the nodes carry proof sentences or only aim text that
describes the shape. Every solved donor run used it; the fresh arm produced
one accepted sentence in 640 turns. But solve rate is the wrong grain for a
rider that finds the template and fumbles execution, and the fresh anchor was
too small - so the discovery is post-hoc until this study confirms it.

Arms: fresh vs concrete donor, n=20 each, five_constraints, 3B + strong
judge, persistent proof channel on.

Pre-registered:
  primary   distinct_accepted per run, concrete > fresh, Mann-Whitney
            |z| > 1.96
  second    solve rate, concrete > fresh, Fisher
  mech      template_hits: accepted sentences matching the donor template
            family (>=0.5 overlap with a donor proof OR the structural
            regex "^(was|is|are) the .*, or not\?$" with a colour and a
            time word) - must be the majority of concrete accepts
  gates     donor arm inherits 28 nodes; fresh stays graph-empty
...
def score_transcript(session_id):
    ...
    import re as _re
    _shape = _re.compile(r'^(was|is|are) the .*, or not\?$', _re.I)
    hits = sum(1 for a in accepted
               if any(CR.overlap(a, p) >= 0.5 for p in PROOFS)
               or _shape.match(a.strip().strip('"')))
    return {'proposals': len(accepted) + len(rejected),
            'accepted': len(accepted), 'rejected': len(rejected),
            'distinct_accepted': len(keeper.distinct_accepted(history)),
            'rode_proof': rode, 'template_hits': hits}
...
def main():
    ...
    ap.add_argument('--out', default='studies/results/v3_floorlift.json')
