#!/usr/bin/env python3
"""Confirmatory replication: the aim scaffold vs no scaffold, and nothing else.

The scaffold ablation found the largest effect of the whole investigation:
aimless weak agents loop (near-dup 0.32, streaks of 9) and barely solve
(1/12), while scaffolded ones loop an order of magnitude less and solve ~40%.
Five subsequent mechanism variants all nulled, so this is the effect the
original concept lives on - and it rests on one study. This is its
pre-registered replication: the decisive two arms only, higher n, nothing
else varied.

  off       aim_system off - no aim, no judge, no graph writes
  scaffold  the full original system - aims, judge, graph memory on

Pre-registered (both must hold):
  1. looping: near_dup_rate off > scaffold, Mann-Whitney |z| > 1.96
  2. solve:   scaffold > off, Fisher p < 0.05
Everything else (turns, distinct_accepted, redo) reported as description.
...
ARMS = ['off', 'scaffold']
ARM_SETTINGS = {
    'off': dict(aim_system='off', graph_memory_mode='none'),
    'scaffold': dict(aim_system='on', graph_memory_mode='graph'),
}
...
def run_one(arm, label, max_calls):
    ...
    s.post(f"{BASE}/update_user_settings", data=dict(
        ...
        aim_system=cfg['aim_system'], aim_fork_mode='gate', task_variant='',
    ...
    if (got.get('aim_system', 'on') != cfg['aim_system']
            or got.get('graph_memory_mode') != cfg['graph_memory_mode']):
        sys.exit(f'server did not accept arm {arm!r} (reports '
                 f'{got.get("aim_system")!r}/{got.get("graph_memory_mode")!r})')
...
def report(rows):
    ...
    print(f"{'arm':<7}{'solved':<9}{'turns/solve':<13}{'capped':<8}"
          f"{'acc':<6}{'rej':<6}{'redo':<6}{'near-dup':<10}{'loop':<6}"
          f"{'distinct3':<11}{'in tok/turn':<12}")
    for arm in ARMS:
        ...
        print(f"{arm:<7}{solve_cell:<9}{str(when):<13}{cap:<8}"
              ...
              f"{str(m('redo_rejected')):<6}{str(m('near_dup_rate', 3)):<10}"
              f"{str(m('longest_loop')):<6}{str(m('distinct_tri', 3)):<11}"
              f"{round(sum(r['input_tokens'] for r in rs) / turns):<12}")

    off = [r for r in rows if r['arm'] == 'off']
    if off and any(r['aims_seen'] for r in off):
        print('\n!! the off arm recorded aims - the ablation did not hold.')
    sc = [r for r in rows if r['arm'] == 'scaffold']
    if sc and not any(r['aims_seen'] for r in sc):
        print('\n!! scaffold: no aims recorded - the scaffold never ran.')
...
def main():
    ...
    ap.add_argument('--out', default='studies/results/v3_replicate.json')
