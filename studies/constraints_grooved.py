#!/usr/bin/env python3
"""Grooved candidate ranking vs similarity top-k: does showing the groove pay?

Follow-up to the transfer study, whose one significant result was harm: a
mismatched inherited graph dropped the weak model from 7/12 to 1/12 because
'similar' candidates are top-k with NO floor - the least-dissimilar wrong
aims still filled the list, and the arbiter trusted the list. 'grooved'
ranking adds a similarity floor, orders candidates nearest-the-goal-by-edges
first and deepest-groove (proven Go/Progress evidence) second, and annotates
all three signals per candidate so the arbiter can see its reasons.

Arms: ranking {similarity, grooved} x donor {fresh, adjacent, mismatch},
n=8 each. Same config as the transfer study otherwise (3B acting, gpt-5.4
judging and arbitrating, two_constraints, window 24).

Pre-registered:
  primary  mismatch-grooved > mismatch-similarity solve rate (harm shrinks)
  guard    fresh-grooved ~= fresh-similarity (no cost where there is no donor)
  second   adjacent-grooved vs adjacent-similarity, direction only
  mech     fork_choice_hops/groove populated in grooved arms; mismatch-grooved
           forks offer fewer donor candidates than mismatch-similarity
...
RANKINGS = ['similarity', 'grooved']
DONOR_ARMS = ['fresh', 'adjacent', 'mismatch']
ARMS = [f'{d}-{r[:4]}' for d in DONOR_ARMS for r in RANKINGS]
...
def run_one(arm, label, max_calls):
    donor_arm, rk = arm.rsplit('-', 1)
    ranking = 'grooved' if rk == 'groo' else 'similarity'
    t0 = time.time()
    s = requests.Session()
    ...
    s.post(f"{BASE}/update_user_settings", data=dict(
        ...
        candidate_ranking=ranking, context_window=str(WINDOW),
        graph_recall_k='6', graph_recall_chars='1200',
    ...
    inherited = 0
    if donor_arm in DONORS:
        r = s.post(f"{BASE}/api/saved_graphs/{DONORS[donor_arm]}/load_into/{doc}",
                   json={'trust': False, 'prune_to': 500}, timeout=180).json()
        inherited = r.get('nodes') or 0
        if not inherited:
            sys.exit(f'donor {DONORS[donor_arm]} loaded 0 nodes into {label}')
    ...
    donor_labels = DONOR_NODES.get(donor_arm, set())
...
def main():
    ...
    ap.add_argument('--donor-adjacent', required=False, default='')
    ap.add_argument('--donor-mismatch',
                    default='graph_4a8002e6-3937-4283-bffd-269a888068a3')
    ap.add_argument('--out', default='studies/results/v3_grooved.json')
    ...
    if not a.donor_adjacent:
        sys.exit('need --donor-adjacent graph id')
    DONORS['adjacent'] = a.donor_adjacent
    DONORS['mismatch'] = a.donor_mismatch
