#!/usr/bin/env python3
"""Does the aim scaffold itself help a weak model? The origin-story ablation.
...
ARMS = ['off', 'none', 'graph']          # aim_system/memory per arm below
ARM_SETTINGS = {
    'off': dict(aim_system='off', graph_memory_mode='none'),
    'none': dict(aim_system='on', graph_memory_mode='none'),
    'graph': dict(aim_system='on', graph_memory_mode='graph'),
}
...
def report(rows):
    ...
    for arm in ('none', 'graph'):
        rs = [r for r in rows if r['arm'] == arm]
        if rs and not any(r['aims_seen'] for r in rs):
            print(f'\n!! {arm}: no aims recorded - the scaffold never ran.')
    g = [r for r in rows if r['arm'] == 'graph']
    if g and not any(r['graph_contribution_chars'] for r in g):
        print('\n!! graph: recall never reached the prompt in any run.')
...
def main():
    ...
    ap.add_argument('--out', default='studies/results/v3_scaffold_w24.json')
